...
NUM_SAMPLES="${PHASE16_MISTRAL_SMOKE_N:-2}"
BASE_CONTEXT_BUDGET="${PHASE16_MISTRAL_BASE_BUDGET:-16384}"
K_VALUES_TEXT="${PHASE16_MISTRAL_K_VALUES:-24 48 96}"
read -r -a K_VALUES <<< "${K_VALUES_TEXT}"
LOG_PATH="${LOG_DIR}/mistral_smoke_$(date -u +%Y%m%dT%H%M%SZ).log"
OUT_CSV="${RESULTS_DIR}/mistral_smoke_n${NUM_SAMPLES}_b${BASE_CONTEXT_BUDGET}.csv"

echo "[phase16-mistral-smoke] start $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee "${LOG_PATH}"
echo "[phase16-mistral-smoke] model=${MODEL_DIR} task=mq_niah_6q_clean_suite B=${BASE_CONTEXT_BUDGET} n=${NUM_SAMPLES} K=${K_VALUES[*]}" | tee -a "${LOG_PATH}"
...
rm -f "${tmp}"
if [[ -z "${artifact}" ]]; then
  echo "[phase16-mistral-smoke] could not locate artifact" | tee -a "${LOG_PATH}"
  exit 1
fi
...
echo "[phase16-mistral-smoke] wrote ${OUT_CSV}" | tee -a "${LOG_PATH}"
echo "[phase16-mistral-smoke] done $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "${LOG_PATH}"
