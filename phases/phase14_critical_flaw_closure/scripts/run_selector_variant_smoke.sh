...
NUM_SAMPLES="${PHASE14_SELECTOR_SMOKE_N:-1}"
...
OUT_CSV="${RESULTS_DIR}/selector_variant_smoke_n${NUM_SAMPLES}.csv"

echo "[phase14-selector-smoke] start $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee "${LOG_PATH}"
echo "[phase14-selector-smoke] task=clean_suite B=16384 n=${NUM_SAMPLES} K=${K_VALUES[*]}" | tee -a "${LOG_PATH}"
...
rm -f "${tmp}"
if [[ -z "${artifact}" ]]; then
  echo "[phase14-selector-smoke] could not locate artifact" | tee -a "${LOG_PATH}"
  exit 1
fi
...
echo "[phase14-selector-smoke] wrote ${OUT_CSV}" | tee -a "${LOG_PATH}"
echo "[phase14-selector-smoke] done $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "${LOG_PATH}"
