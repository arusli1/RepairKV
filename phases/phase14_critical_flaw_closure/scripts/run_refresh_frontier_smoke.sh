...
NUM_SAMPLES="${PHASE14_REFRESH_SMOKE_N:-2}"
K_VALUES_TEXT="${PHASE14_REFRESH_K_VALUES:-24 48 80 96}"
read -r -a K_VALUES <<< "${K_VALUES_TEXT}"
LOG_PATH="${LOG_DIR}/refresh_frontier_smoke_$(date -u +%Y%m%dT%H%M%SZ).log"
OUT_CSV="${RESULTS_DIR}/refresh_frontier_smoke_n${NUM_SAMPLES}.csv"

echo "[phase14-refresh-smoke] start $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee "${LOG_PATH}"
echo "[phase14-refresh-smoke] task=clean_suite B=16384 n=${NUM_SAMPLES} K=${K_VALUES[*]}" | tee -a "${LOG_PATH}"
...
rm -f "${tmp}"
if [[ -z "${artifact}" ]]; then
  echo "[phase14-refresh-smoke] could not locate artifact" | tee -a "${LOG_PATH}"
  exit 1
fi
...
echo "[phase14-refresh-smoke] wrote ${OUT_CSV}" | tee -a "${LOG_PATH}"
echo "[phase14-refresh-smoke] done $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "${LOG_PATH}"
