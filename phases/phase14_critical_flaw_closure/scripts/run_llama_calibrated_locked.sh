...
NUM_SAMPLES="${PHASE14_LLAMA_LOCKED_N:-24}"
...
LOG_PATH="${LOG_DIR}/llama_calibrated_locked_$(date -u +%Y%m%dT%H%M%SZ).log"
OUT_CSV="${RESULTS_DIR}/llama_calibrated_locked_n${NUM_SAMPLES}_b${BASE_CONTEXT_BUDGET}.csv"

echo "[phase14-llama-locked] start $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee "${LOG_PATH}"
echo "[phase14-llama-locked] model=${MODEL_DIR} task=mq_niah_6q_clean_suite B=${BASE_CONTEXT_BUDGET} n=${NUM_SAMPLES} K=${K_VALUES[*]}" | tee -a "${LOG_PATH}"
...
rm -f "${tmp}"
if [[ -z "${artifact}" ]]; then
  echo "[phase14-llama-locked] could not locate artifact" | tee -a "${LOG_PATH}"
  exit 1
fi
...
echo "[phase14-llama-locked] wrote ${OUT_CSV}" | tee -a "${LOG_PATH}"
echo "[phase14-llama-locked] done $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "${LOG_PATH}"
