...
def _spec_signature(task_name: str) -> dict:
...
def _load_or_generate_examples(task_name: str, context_length: int, num_samples: int, tokenizer, force: bool) -> list:
    """Reuse a cached dataset when possible; otherwise regenerate it deterministically."""
    path = dataset_path(task_name, context_length)
    expected_signature = _spec_signature(task_name)
    ...
    examples = [build_task_example(task_name, index, context_length, tokenizer) for index in range(num_samples)]
