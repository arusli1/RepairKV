...
def build_niah_example(task_name: str, index: int, target_context_length: int, tokenizer, *, mode: str, num_needles: int, depths: list[float]) -> TaskExample:
    ...
    rng = make_rng((target_context_length * 1000) + index + len(task_name))
    key_values = []
    if mode == "multi_value":
